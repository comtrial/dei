...
### 4-1. 진입 가드
- [ ] videoId param 으로 `video` row fetch.
- [ ] member_id 가 본인 `block` 관계 있는지 확인 → 있으면 즉시 `router.back()` + 토스트.
- [ ] storage signed URL 생성 → `FullscreenVideo` source 로.

### 4-2. 자동 루프 재생 + **poster** (C-1 §4-2)
- [ ] `expo-video` `useVideoPlayer({ uri, poster: thumbnailUrl })` 또는 `<VideoView posterSource={...} />`.
- [ ] **poster** = S13 grid 에서 받은 동일한 썸네일 jpg → 로드 전·로드 중 검은 화면 0건.
- [ ] `shouldLoop=true`, `isMuted` default true.
- [ ] 가로 영상 (letterbox or 자동 가로 회전 — DS 결정).
- [ ] `bufferOptions.preferredForwardBufferDuration: 1` (1초만 미리, C-1 §4-3).
- [ ] `ProgressBar` 상단 — `playbackStatus.positionMillis / durationMillis` 매핑.

### 4-3. 좌·우 swipe (같은 시간대 다른 멤버) + **prefetch** (C-1 §4-4)
- [ ] 진입 시 같은 hour_slot 의 sibling video 목록 prefetch.
- [ ] **현재 + 1 (다음 swipe 후보) 만 적극 prefetch** — 전부 prefetch 시 데이터 폭증:
  - `expo-image` `Image.prefetch([thumbnailUrl_next])` (썸네일).
  - `expo-video` 직전·직후 player 인스턴스 미리 생성 (poster 표시 + 첫 1초만 fetch).
- [ ] PanResponder 또는 `react-native-pager-view` 로 좌·우 swipe.
- [ ] 양 끝 = bounce 후 stop (관례).
- [ ] 시간대 이동은 여기 X — S13 timestrip 에서만.
- [ ] swipe 후 새 video 의 첫 frame ≤ 400ms (prefetch 효과 측정).

### 4-4. 영상 영역 탭 / 길게 누르기 (Reels 패턴)
- [ ] 단일 탭 = 일시 정지 토글 + 일시정지 인디케이터 표시.
- [ ] 길게 누르기 = 일시 정지 + UI 숨김 (×, 멤버 칩, ProgressBar 모두 숨김).
- [ ] 손 떼면 UI 복귀 + 재생.

### 4-5. 닫기 / 멤버 칩
- [ ] 좌상단 × → `router.back()` (S13 으로).
- [ ] 우상단 멤버 칩 (Chip + Avatar) 탭 → `/(app)/room/[roomId]/members?userId=...` (S14).

### 4-6. fetch 실패
- [ ] storage signed URL 실패 → `StateView` "영상을 불러오지 못했어요" + 재시도 버튼.
