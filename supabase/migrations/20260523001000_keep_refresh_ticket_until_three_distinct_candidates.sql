-- Keep refresh tickets until the server can return three distinct curation users.
-- A user can have multiple approved videos in curation_pool; counting rows here
-- would consume a ticket even when fewer than three people are available.
