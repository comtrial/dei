-- Relax rematch cooldown enforcement to the male team owner.
--
-- enqueue-match-queue already gates rematch eligibility by the inviting owner.
-- match_and_create used to require every restricted male participant to own a
-- booster pass, so an invited friend who had just left a room could block the
-- whole team even when the host was eligible. The paid-rematch rule now follows
-- the product rule: only the male side's team owner must be outside the 12h
-- cooldown or spend one booster pass.
...
begin
  ...
  v_male_team := case
    when p_side_a_gender = 'male' then v_team_a
    when p_side_b_gender = 'male' then v_team_b
    else null
  end;

  if v_male_team is not null then
    select t.owner_user_id into v_restricted_owner
      from public.team t
      join public.profile p on p.user_id = t.owner_user_id
      where t.id = v_male_team
        and p.gender = 'male'
        and p.last_room_leave_at is not null
        and p.last_room_leave_at > now() - interval '12 hours'
      limit 1;

    if v_restricted_owner is not null then
      ...
      where p.id = (
        select p2.id
        from public.pass p2
        where p2.user_id = v_restricted_owner
          and p2.kind = 'booster'
